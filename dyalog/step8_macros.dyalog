...
:Namespace m
  ...
  mkNumFn←{
    ...
    nonNumber←(N=⊃¨⍵)⍳0
    nonNumber>⍴⍵: N (⍺⍺ (⊃1∘↓)¨⍵)
                  E.TypeError E.throw N (nonNumber⊃⍵)
  }
  ...
  eval←{
    ...
    macroexpand←{
      ...
      res fn←⍺isMC⍵
      ~res: ⍵
      newForm←core.cons fn (core.list quote¨SE(2⊃core.cdr⍵))
      ast←⍺eval newForm
      ast
    }
    ...
    ty≠T.Function: E.TypeError E.throw (T.Function T.Builtin) (ty F)
  ...
  ∇R←env rep input
   Errors←E.(TypeError NameError IndexError UserError)
   :Trap Errors
