...
:Namespace m
  ...
  L V M S Str K Fun BFun←T.(List Vec Map Symbol String Keyword Function Builtin)
  ...
  eval←{
    ...
    S 'defmacro!'≡head: ⍺{
      name mFn←tail
      t v←2↑val←⍺ eval mFn
      t≢Fun: typeError Fun val
      v←⎕ns v
      v.isMacro←1
      _←(((2⊃name) Env.def (Fun v,2↓val)) ⍺)
      val
    }⍬
    ...
    S 'apply-internal'≡head: ⍺{
      fval A←⍺eval¨tail
      t f←2↑fval                  ⍝ takes 2 args, f and lst
      last←2⊃⊃¯1↑2⊃A              ⍝ lst is a list (a b c (more-args))
      butlast←¯1↓2⊃A              ⍝ A: arglist, in the form (a b c ~@more-args)
      A←butlast,last
      ~t∊Fun BFun: typeError (Fun BFun) fval
    ...
    ty≠Fun: typeError (Fun BFun) (ty F)
  ...
  init←{
    ...
    _←GLOBAL eval read '(println (str "Mal [" *host-language* "]"))'
    ⍬
  }
