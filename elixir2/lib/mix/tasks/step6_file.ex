defmodule Mix.Tasks.Step6File do
  ...
  def core_env do
    {:ok, env_pid} = Mal.Env.start_link
    env = Mal.Env.new(env_pid)
    ...
    core_ns = %{
      ...
      :"pr-str" => &pr_str/1,
      :str      => &str/1,
      :"read-string" => fn ([s]) -> Mal.Reader.read_str(s) end,
      :slurp    => &slurp/1,
      :eval     => fn ([form]) -> eval(form, env) end
    }
    core_ns
    |> Enum.each(fn ({k, v}) -> Mal.Env.put(env, k, v) end)

    read("(def! load-file (fn* [path] (eval (read-string (slurp path)))))")
    |> eval(env)
